...
let CachedRequestsExpirationTime = 100;
...
class CachedRequestsManager {
    static add(url, content, ETag = "") {
        if (url != "") {
            CachedRequests.push({ url, content, ETag, expireIn: utilities.nowInSeconds() + CachedRequestsExpirationTime });
        ...
        try {
            if (url != "") {
                for (let endpoint of CachedRequests) {
                    if (endpoint.url == url) {
                        // renew cached url
                        endpoint.expireIn = utilities.nowInSeconds() + CachedRequestsExpirationTime;
